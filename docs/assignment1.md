...
For a text-adventure type of game, we cannot get around the use of commands, however, initially we thought of too many different commands to also properly support the idea of configurability. This is why we decided to narrow down the list of command to a set of commands which can be implemented and interpreted in a general sense. Based on the commands we agreed to use, we developed all other functional features. Both movement and areas are fundamental functional features, since they allow us to create a setting for some story to take place in, thus further supporting the idea of being able to configure the game. Our choice for representing the areas as nodes in a graph arose pretty early on, since it seemed intuitive to us that this approach would greatly simplify connecting and moving between areas. To make the game actually interesting and interactive, we concluded that having items and NPCs as functional features would make most sense, since they are easily modifiable and therefore provide us with a great amount of freedom. To put this in perspective, both a sword and a car may be different items used in different game configurations, but with the same underlying implementation. We also decided to make combat and stats functional features to further the idea of an adventure type of game in which there is also an element of risk involved. Finally, we came up with the functional features of saving and loading gamestates. These features do not necessarily affect the game itself, however, they do offer another challenge for us.

| ID  | Short name  | Description  |
|---|---|---|
| F1  | Commands  | The player can control the main character by issuing command-line commands following this syntax: `command + [target]`, where (\*) indicates a target is optional. Available commands are: <br/> - `move` + [north, east, south, west, area-name] <br/> - `use` + [item] + [npc, item, obstacle]* <br/> - `look` + [item]\* <br/> - `interact` + [npc, item] <br/> - `inventory` <br/> - `save` <br/> - `load` <br/> - `quit` <br/> |
...
| F4  | Obstacles | An obstacle is a special type of object which prohibits movement between two specific areas until a certain requirement is met. Obstacles are similar to, but not a subset of, items in the sense that they may be interacted with, and items may be used on them. As a bonus feature NPCs may also act as an obstacle, preventing the player from advancing unless they perform some task for them or defeat them in combat.  |
| F5  | Items | The player may acquire and use items throughout the game. Items will be stored in an inventory of infinite size, which may be accessed using the `inventory` command. Some items are consumed upon first use, whereas other items persist throughout the game. Several commands may be used in combination with an item: </br> - `interact` acquires the item and places it in the player's inventory. </br> - `look` briefly describes the item. </br> - `use` queries the item, asking it what it may be used for, and if appropriate for the situation, uses it (e.g. `use key door` will consume the key but `use key npc` will not).  |
...
| F7  | Stats | The player has two main stats, `health` and `strength`, which they will use for combat. Naturally, `health` is an indicator of the player's vitality whereas `strength` affects the player's ability to fight certain NPCs.  |
| F8  | Combat | Whenever a player engages in combat, their strength is compared to that of the enemy combatant. The outcome of the fight is determined by the difference in strength between both the player and their enemy:</br> - player strength < enemy strength: player dies</br> - player strength >= enemy strength: player wins but loses health</br> - player strength >> enemy strength: player wins  |
| F9  | Gamestate | The player may save or load the game at any moment using the `save` and `load` commands respectively. A savefile will contain all actions as performed by the player up until the point of saving, along with the appropriate game configurations. Whenever a savefile is loaded into the game, all actions are replayed so the user may proceed where they left off when saving. The player may quit the game after saving or loading, using the `quit` command at any time. Otherwise, the game will end when some predetermined win condition has been met, or alternatively, when the player has died.  |
