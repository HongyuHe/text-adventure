...
- The system will wait for the user to provide an input in the form of supported commandBlueprints.
...
| F9  | Gamestate | The player may save or load the game at any moment using the `save` and `load` commandBlueprints respectively. A savefile will contain all actions as performed by the player up until the point of saving, along with the appropriate initial game configuration. Whenever a savefile is loaded into the game, all actions are replayed so the user may proceed where they left off when saving. The player may quit the game after saving or loading, using the `quit` command at any time.  |
...
| QR3 | Event Timing | Availability | A command can only be issued by the user once the events of the previous command have been completed. | This ensures that the execution of commandBlueprints will always be deterministic and helps avoid any potential confusion that the player may face (e.g. receiving responses from commandBlueprints out of order). |
